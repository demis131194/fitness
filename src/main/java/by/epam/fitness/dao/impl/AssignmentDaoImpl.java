...
public class AssignmentDaoImpl implements AssignmentDao {
    ...
    private static final String INSERT_QUERY = "INSERT INTO assignments (orderId, exercises, nutrition, startDate, endDate, price) VALUES (?, ?, ?, ?, ?, ?)";
    private static final String UPDATE_QUERY = "UPDATE assignments SET orderId = ?, exercises = ?, nutrition = ?, startDate = ?, endDate = ?, price = ? WHERE id = ?";
    private static final String DELETE_QUERY = "UPDATE assignments SET active = false WHERE id = ?";
    private static final String FIND_ACTIVE_QUERY = "SELECT id, orderId, registerDate, exercises, nutrition, startDate, endDate, price, accept FROM assignments WHERE id = ? AND active = true";
    ...
    public Assignment create(Assignment assignment) throws DaoException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(INSERT_QUERY, Statement.RETURN_GENERATED_KEYS)) {
            ...
            statement.setString(2, assignment.getExercises());
            statement.setString(3, assignment.getNutrition());
            statement.setDate(4, Date.valueOf(assignment.getStartDate()));
            statement.setDate(5, Date.valueOf(assignment.getEndDate()));
            statement.setInt(6, assignment.getPrice());
            statement.execute();
    ...
    @Override
    public List<Assignment> findAllActive() throws DaoException {
